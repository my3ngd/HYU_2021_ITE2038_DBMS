...
int64_t file_open_table_file(char* pathname)
{
     ...
     */
    int64_t table_id = open(pathname, O_RDWR | O_CREAT | 0, S_IRWXU | S_IRWXG | S_IRWXO);
    ...
    if (!cur->num_of_pages || tmp != 4096)
    {
        ...
        for (int i = 1; i < INIT_PAGE_NUM; i++)
        {
            ...
            pwrite(table_id, tmp, PAGE_SIZE, PAGE_SIZE*i);
        }
        sync();
        // header 0 -> 1 -> 2 ... -> 2559 -> 0
    }
...
pagenum_t file_alloc_page(int64_t table_id)
{
    ...
    if (!cur_num)
    {
        ...
        for (int i = cur_num; i < header_page->num_of_pages; i++)
        {
            ...
            pwrite(table_id, cur_page, PAGE_SIZE, PAGE_SIZE*i);
        }
        sync();
