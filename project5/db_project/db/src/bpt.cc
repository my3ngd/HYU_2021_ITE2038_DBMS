...
void print_leaves(int64_t tid)
{
    ...
    buffer_get_header(tid, header);
    page_t cur;
    pagenum_t cur_num = header.root_num;
    buffer_read_only_page(tid, cur_num, cur);
    ...
    // print and go right
    for (memory_leaf_t ml; cur_num; buffer_read_only_page(tid, cur_num, cur))
    {
...
int get_height(int64_t tid, pagenum_t page_num)
{
    ...
    page_t cur;
    buffer_read_only_page(tid, page_num, cur);
...
void print_tree(int64_t tid)
{
    ...
    while (q.size())
    {
        ...
        if (cur_h != tmp_h)     {fs << endl;tmp_h = cur_h;}
...
int db_find(int64_t tid, int64_t key, char *ret_val, uint16_t* val_size)
{
    ...
    if (!leaf_num)  return 1;

    buffer_read_only_page(tid, leaf_num, leaf);
 ...
 */
void insert_into_leaf(int64_t tid, pagenum_t leaf_num, record_t rec)
{
    page_t leaf;
    buffer_read_page(tid, leaf_num, leaf);
    ...
    buffer_write_page(tid, leaf_num, leaf);
 ...
 */
void insert_into_leaf_after_splitting(int64_t tid, pagenum_t lleaf_num, record_t rec)
{
    page_t rleaf(1), lleaf;
    buffer_read_page(tid, lleaf_num, lleaf);
    pagenum_t rleaf_num = buffer_alloc_page(tid);
    ...
    buffer_write_page(tid, lleaf_num, lleaf);
    buffer_write_page(tid, rleaf_num, rleaf);

    header_page_t header;
    buffer_get_header(tid, header);

    return insert_into_parent(tid, lml.header.parent_num, lleaf_num, rleaf_num, key_up);
}
 ...
 */
void insert_into_parent(int64_t tid, pagenum_t pa_num, pagenum_t le_num, pagenum_t ri_num, int64_t key)
{
    // case 0: new root
    if (!pa_num)
        return insert_into_new_root(tid, le_num, ri_num, key);
    
    page_t pa(0);
    buffer_read_only_page(tid, pa_num, pa);
    int insert_idx = get_insert_idx(tid, pa, le_num);

    if (pa.header.num_of_keys < BRANCHING_FACTOR-1)
        insert_into_internal(tid, pa_num, ri_num, key, insert_idx);
    else
        insert_into_internal_after_splitting(tid, pa_num, ri_num, key, insert_idx);
}

/* insert_into_internal
 * not work recursivly
 */
void insert_into_internal(int64_t tid, pagenum_t pa_num, pagenum_t ri_num, int64_t key, int idx)
{
    page_t pa, ri;
    buffer_read_page(tid, pa_num, pa);
    buffer_read_page(tid, ri_num, ri);
    // shift
    ...
    // save
    buffer_write_page(tid, pa_num, pa);
    buffer_write_page(tid, ri_num, ri);
 ...
 * not work recursivly
 */
void insert_into_internal_after_splitting(int64_t tid, pagenum_t pa_num, pagenum_t ri_num, int64_t key, int idx)
{
    page_t pa, ri;
    buffer_read_page(tid, pa_num, pa);
    ...
    pagenum_t ch_num = ne.ip.left_num,
              ne_num = buffer_alloc_page(tid);
    buffer_write_page(tid, pa_num, pa);
    buffer_write_page(tid, ne_num, ne);

    // about childs
    // ne's leftmost child
    buffer_read_page(tid, ch_num, ch);
    ch.header.parent_num = ne_num;
    buffer_write_page(tid, ch_num, ch);
    // ne's other children
    ...
    return insert_into_parent(tid, pa.header.parent_num, pa_num, ne_num, tmp_edge[split].key);
}

/* insert_into_new_root
 * called when need to split old-node
 */
void insert_into_new_root(int64_t tid, pagenum_t le_num, pagenum_t ri_num, int64_t key)
{
    page_t le, ri;
    buffer_read_page(tid, le_num, le);
    buffer_read_page(tid, ri_num, ri);

    page_t pa(0);   // must be internal node
    pagenum_t pa_num = buffer_alloc_page(tid);
    header_page_t header;
    buffer_get_header(tid, header);
    // about header page
    ...
    // save
    buffer_write_page(tid, pa_num, pa);
    buffer_write_page(tid, le_num, le);
    buffer_write_page(tid, ri_num, ri);

    buffer_set_header(tid, header);
    return ;
}
...
{
    // about root page
    pagenum_t root_num = buffer_alloc_page(tid);   // alloc page
    insert_into_leaf(tid, root_num, rec);
    // $ direct buffer function call: necessary.. $
    buffer->head->next->pinned = false;
...
int db_insert(int64_t tid, int64_t key, char* value, uint16_t val_size)
{
    ...
    if (val_size+12 <= mleaf.free_space) // 12: slot size
        insert_into_leaf(tid, leaf_num, record_t(key, value, val_size));
    // case 3: or not
    else
        insert_into_leaf_after_splitting(tid, leaf_num, record_t(key, value, val_size));
    return 0;
}
...
{
    page_t pa;
    buffer_read_only_page(tid, page.header.parent_num, pa);
 ...
 */
void remove_entry_from_node(int64_t tid, pagenum_t page_num, int64_t key)
{
    page_t page;
    buffer_read_page(tid, page_num, page);
    int idx = 0;
    // leaf case
    ...
    // save & end
    buffer_write_page(tid, page_num, page);
    return ;
}
 ...
 */
void adjust_root(int64_t tid, pagenum_t root_num)
{
    page_t root;
    buffer_read_page(tid, root_num, root);

    // case 0: normal case (tree exists, root node not changes)
    if (root.header.num_of_keys > 0)
        return buffer_write_page(tid, root_num, root);
    ...
    if (!root.header.is_leaf)
    {
        ...
        buffer_read_page(tid, new_root_num, new_root);
        new_root.header.parent_num = 0;
        buffer_write_page(tid, new_root_num, new_root);
    }
    // case2: empty tree; run below code
    header.root_num = new_root_num;
    buffer_set_header(tid, header);
    buffer_free_page(tid, root_num);
    return ;
}
 ...
 */
void redistribute_leaf(int64_t tid, pagenum_t my_num, pagenum_t ne_num, int my_idx, int ne_idx)
{
    page_t pa, my, ne;
    buffer_read_page(tid, my_num, my);
    buffer_read_page(tid, ne_num, ne);
    ...
    // save
    buffer_write_page(tid, pa_num, pa);
    my = memleaf_to_diskleaf(mmy);  buffer_write_page(tid, my_num, my);
    ne = memleaf_to_diskleaf(mne);  buffer_write_page(tid, ne_num, ne);
    if (mmy.free_space >= THRESHOLD)
        return redistribute_leaf(tid, my_num, ne_num, my_idx, ne_idx);
    return ;
}
 ...
 */
void redistribute_internal(int64_t tid, pagenum_t my_num, pagenum_t ne_num, int my_idx, int ne_idx)
{
    page_t pa, ch, my, ne;
    buffer_read_page(tid, my_num, my);
    buffer_read_page(tid, ne_num, ne);
    ...
    pa = meminternal_to_diskinternal(mpa);  buffer_write_page(tid, pa_num, pa);
    my = meminternal_to_diskinternal(mmy);  buffer_write_page(tid, my_num, my);
    ne = meminternal_to_diskinternal(mne);  buffer_write_page(tid, ne_num, ne);
 ...
 * "comment"
 */
void merge_leaf(int64_t tid, pagenum_t my_num, pagenum_t ne_num, int my_idx, int ne_idx)
{
    page_t pa, my, ne;
    buffer_read_page(tid, my_num, my);
    buffer_read_page(tid, ne_num, ne);
    ...
    if (my_idx < 0)
    {
        ...
        buffer_write_page(tid, my_num, my);
        buffer_free_page(tid, ne_num);
    }
...
/* merge_internal
 */
void merge_internal(int64_t tid, pagenum_t my_num, pagenum_t ne_num, int my_idx, int ne_idx)
{
    page_t pa, ch, my, ne;
    buffer_read_page(tid, my_num, my);
    buffer_read_page(tid, ne_num, ne);
...
{
    remove_entry_from_node(tid, page_num, key);
    ...
    if (page_num == header.root_num)
        return adjust_root(tid, page_num);

    page_t my;
    buffer_read_only_page(tid, page_num, my);
    // parent node
    page_t pa;
    pagenum_t pa_num = my.header.parent_num;
    buffer_read_only_page(tid, pa_num, pa);
    ...
    buffer_read_only_page(tid, ne_num, ne);
    ...
    if (my.header.is_leaf)
    {
        ...
        if (my.lp.free_space < THRESHOLD)
            return ;//(void)buffer_write_page(tid, page_num, my);
        // case 1-1: merge
        else if (INIT_FREE_SPACE - my_free <= ne_free)
            return merge_leaf(tid, page_num, ne_num, my_idx, ne_idx);
        // case 1-2: redistribution
        else
            return redistribute_leaf(tid, page_num, ne_num, my_idx, ne_idx);
    }
    ...
    else
    {
        ...
        // case 2-1: merge
            ...
            return merge_internal(tid, page_num, ne_num, my_idx, ne_idx);
        // case 2-2: redistribution
        else
            return redistribute_internal(tid, page_num, ne_num, my_idx, ne_idx);
    }
}
