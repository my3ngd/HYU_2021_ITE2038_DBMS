...
bool edge_t::operator==(const edge_t& e)
{
    return (this->key == e.key) && (this->child_num == e.child_num);
}
...
bool page_header_t::operator==(const page_header_t& p)
{
    if (this->parent_num != p.parent_num)   return false;
    if (this->is_leaf != p.is_leaf) return false;
    if (this->num_of_keys != p.num_of_keys) return false;
    return true;
}
...
bool page_t::operator==(const page_t& p)
{
    if (this->header != p.header)   return false;
    // leaf equal
    if (this->header.is_leaf)
    {
        if (this->lp.free_space != p.lp.free_space) return false;
        if (this->lp.right_num != p.lp.right_num) return false;
        // we can 'assume' that if free space is equal, slots and records may equal too.
    }
    else
    {
        if (this->ip.left_num != this->ip.left_num) return false;
        for (int i = 0; i < this->header.num_of_keys; i++)
            if (this->ip.edges[i] != p.ip.edges[i]) return false;
    }
    return true;
}
